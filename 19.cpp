...
{
	printf("請輸入一連串(最多10個)矩形的左上及右下座標,輸入完後請輸入y進入下一步\n");  
        ...
        m++;
...

          {
            ...
            if(cr[a]==1&&x>map[a][0]&&x<map[a][2]&&y<map[a][1]&&y>map[a][3]) 
              ...
              {printf("The result is：Point %d is contained in figure %d\n",m,a+1);flag=1;}   
            ...
            if(flag==0)
               ...
               printf("The result is：Point %d is not contained in any figure\n",m);  
